...
# usage:
# python 1D.2Epoch.dadi.py --runNum $i --pop CA --mu [mutation rate] --L [total sites including all monomorphic!] --sfs [path to sfs] --outdir [path to outdir]
...
# SET YOUR UPPER AND LOWER BOUNDS and INITIAL PARAMS APPROPRIATELY <---- !!!!!
upper_bound = [10, 1]
lower_bound = [1e-4, 1e-5]
p0 = [0.1,0.1] # initial parameters
...
Nanc=theta / (4*mu*L)
nu_scaled_dip=popt[0]*Nanc # sizes are scaled by Nanc
T_scaled_gen=popt[1]*2*Nanc  # times are scaled by 2Nanc
